...
class Photo_Mosaic:
    def __init__(self,original_image,
        ...
        self.tile_rescale = tile_rescale
        self.tile_size = tile_size
        self.large_tile_data,self.small_tile_data = Tile_Processor(tile_directory,tile_size,tile_rescale,
                                                                               img_type=img_type).get_data()
        self.print_progress = print_progress
        self.char_matrix=[]
    ...
    def update_time(self):
        ...
        tiles = len(self.large_image_data) // max(self.threads, 1)
        calc = TileCalculator()
        calc.update(tiles, elapsed_time)

    def set_data(self):
        tiles = Tile_Processor(self.tile_directory,self.tile_size,self.tile_rescale,
                               img_type=self.img_type)
        return tiles.get_data()
